...
\mid ![](/pictures/prml/图9.1.png)\mid 
\mid :--:\mid 
\mid 在⽼忠实间歇喷泉数据集上进行 K 均值聚类。(a)数据集和分类中心 $\mu_1$ 和 $\mu_2$ (b)在初始的E步骤中，每个数据点根据与哪个中⼼更近来确定类别。这等价于根据两个聚类中⼼的垂直平分线来对数据点进⾏分类。(c)在接下来的M步骤中，每个聚类中⼼使⽤分配到对应类别的数据点重新计算。(d)-(i)给出了接下来的E步骤和M步骤，直到最终收敛。\mid 
...
\mid ![](/pictures/prml/图2.22.png)\mid 
\mid :-:\mid 
\mid ⼀维⾼斯混合分布的例⼦\mid 
\mid ![](/pictures/prml/图2.23.png)\mid 
\mid ⼆维空间中3个⾼斯分布混合的例⼦\mid 
\mid ![](/pictures/prml/图2.21.png)\mid 
\mid 这个数据集被称为“⽼忠实间歇喷泉”数据集，由美国黄⽯国家公园的⽼忠实间歇喷泉的272次喷发的测量数据组成。可以看到使用最⼤似然法（左）拟合的单一高斯分布不能描述数据中两个聚集区域，峰值区域内数据稀疏。而使用两个高斯分布线性组合得到的概率分布（右）给出了数据的一个更好的表示。\mid 
...
\mid ![](/pictures/prml/图9.4.png)\mid 
\mid :-:\mid 
\mid 混合模型的图形表⽰，其中联合概率分布被表⽰为 $p(x,z)=p(x\mid z)$  的形式。\mid 
...
\mid ![](/pictures/prml/图9.5.png)\mid 
\mid :-:\mid 
\mid 从3个⾼斯分布组成的混合分布中抽取的 500 个样本点。(a)从联合概率分布中抽取的样本， 其中 $z$ 的三种状态对应于混合分布的三个分量， ⽤红⾊、绿⾊、蓝⾊表⽰。(b)来⾃边缘概率分布 $p(x)$ 的对应的样本，仅仅将 $z$ 的值忽略，画出 $x$ 的值即可。(a)中的数据集被称为完整的，(b)中的数据集被称为不完整的。(c)同样的样本，颜⾊表⽰与数据点 $x_n$ 关联的责任 $\gamma(z_{nk})$，其中红⾊、蓝⾊、绿⾊所占的⽐重分别由 $\gamma(z_{nk}), k = 1, 2, 3$ 给出。\mid 

假设我们有⼀个观测的数据集 $\{x_1,\dots,x_n\}$，将这个数据集表⽰为⼀个 N × D 的矩阵 $X$ ，其中第 $n$ ⾏为 $x_n^T$ 。类似地，对应的隐含变量会被表⽰为⼀个 N × K 的矩阵 $Z$ ，它的⾏为 $z_n^T$. 图模型表示如下：

\mid ![](/pictures/prml/图9.6.png)\mid 
\mid :-:\mid 
\mid  一组 $N$ 个独立同分布数据点 $\{x_n\}$ 的高斯混合模型的图表示，对应的潜在变量为 $\{z_n\}$, $n=1,\dots,N$ \mid  
...
\mid ![](/pictures/prml/图1.14.png)\mid ![](/pictures/prml/图9.7.png)\mid 
\mid :-:\mid :-:\mid 
\mid 红⾊曲线表⽰单⼀的⾼斯概率分布的似然函数。⿊点表⽰数据集 $\{x_n\}$，似然函数对应于蓝⾊值的乘积。\mid  混合⾼斯模型中出现的似然函数的奇异性。⼀个分量会收缩到⼀个具体的数据点，因此会给对数似然函数贡献⼀个不断增加的值。\mid 
...
\mid ![](/pictures/prml/图9.8.png)\mid 
\mid :-:\mid 
\mid  对⽼忠实间歇喷泉数据集使⽤EM算法。图(a)⽤绿⾊标记出了数据点，以及初始的混合模型的配置。两个⾼斯分量的⼀个标准差位置的轮廓线分别⽤红⾊圆圈和蓝⾊圆圈标记。图(b)给出了初始E步骤的结果。图(c)给出了第⼀个M步骤之后的结果。图(d),(e)和(f)分别给出了2次、5次、20次完整的EM循环之后的结果。在图(f)中，算法接近收敛。 \mid  
