...
现在**假定对于$ X $中的每个观测，我们都有潜在变量$ Z $的对应值。我们将$ {X, Z} $称为完整（complete）数据集，并且我们称实际的观测数据集 $X$ 是不完整的（incomplete）**，如图9.5所示。完整数据集的对数似然函数的形式为$ \ln p(X, Z|\theta) $，并且我们假定对这个完整数据的对数似然函数进行最大化是很容易的。
