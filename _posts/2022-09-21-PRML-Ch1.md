...
$\|w\|^2 =w^T w=w^2_0 + w^2_1 + ... + w^2_M$ 系数$\lambda$控制了正则化项相对于平方和误差项的重要性。
...
加法法则 $$p(X)=\sum_{Y}p(X,Y) \tag{1.10}$$
乘积法则 $$p(X,Y)=P(Y|X)p(X) \tag{1.11}$$
    
根据这两个式子得到本书最重要的定义：贝叶斯定理。
    
$$P(Y | X)=\frac{P(X | Y) P(Y)}{P(X)} \tag{1.12}$$
    
分子中的$P(x)=\sum_{Y}P\left(X | Y\right)P\left(Y\right)$其实这就是加法规则。确保了概率在归一化之后的值为1。
...
频率学家观点: w 被认为是⼀个固定的参数，频率学家⼴泛使⽤的⼀个估计是最⼤似然（maximum likelihood）估计，其中 w 的值是使似然函数p(D | w) 达到最⼤值的 w 值，这个估计的误差通过考察可能的数据集 D 的概率分布来得到。⼀种决定频率学家的误差的⽅法是⾃助法（bootstrap）。

贝叶斯观点: 只有⼀个数据集 D（即实际观测到的数据集），参数w是不固定的，不确定性通过w的概率分布来表达。
...
$$
\mathcal{N}(\mathbf{x} \mid \boldsymbol{\mu}, \boldsymbol{\Sigma})=\frac{1}{(2 \pi)^{D / 2}} \frac{1}{|\boldsymbol{\Sigma}|^{1 / 2}} \exp \left\{-\frac{1}{2}(\mathbf{x}-\boldsymbol{\mu})^{\mathrm{T}} \boldsymbol{\Sigma}^{-1}(\mathbf{x}-\boldsymbol{\mu})\right\} \tag{1.52}
$$

其中D维向量 µ 被称为均值，D × D的矩阵Σ被称为协⽅差， | Σ | 表⽰Σ的⾏列式。

现在假定我们有⼀个观测的数据集$x = (x_1, \dots , x_N)^T$，我们假定各次观测是独⽴地从⾼斯分布中抽取的，分布的均值 µ 和⽅差 σ2 未知，我们想根据数据集来确定这些参数。独⽴地从相同的分布中抽取的数据点被称为独⽴同分布（independent and identically distributed），通常缩写成i.i.d.。

给定 µ 和 σ 2 ，我们可以给出数据集的概率

$$
p(x\mid\mu,\sigma) = \prod_{n=1}^{N}\mathcal{N}(x_n\mid,\mu,\sigma^2) \tag{1.53}
$$

当我们把它看成 µ 和 σ 2的函数的时候，这就是⾼斯分布的**似然函数**。根据公式（1.46）和 公式（1.53），对数似然函数可以写成
$$
\ln p(x\mid\mu,\sigma^2) = \sum_{n=1}^{N}\ln(\frac{1}{\left(2 \pi \sigma^{2}\right)^{\frac{1}{2}}} \exp \left\{-\frac{1}{2 \sigma^{2}}(x_n-\mu)^{2}\right\}) 
=-\frac{1}{2\sigma^2}\sum_{n=1}^{N}(x_n-\mu)^2+\sum_{n=1}^{N}\ln(2\pi\sigma^2)^{-\frac{1}{2}}\\
=-\frac{1}{2\sigma^2}\sum_{n=1}^{N}(x_n-\mu)^2-\frac{N}{2}\ln(2\pi)-\frac{N}{2}\ln\sigma^2 \tag{1.54}
$$
我们可以得到关于 µ最⼤似然解
$$
\mu_{ML} = \frac{1}{N}\sum_{n=1}^{N} x_n  \tag{1.55}
$$
这是样本均值（sample mean），即观测值 { x n} 的均值。

类似地，关于 $\sigma^2$ 最⼤化函数（1.54）， 我们得到了⽅差的最⼤似然解
$$
\sigma^2_{ML} = \frac{1}{N}\sum_{n=1}^{N}(x_n-\mu_{ML})^2 \tag{1.56}
$$
这是关于样本均值 µ ML 的样本⽅差（sample variance）。 注意， 我们要同时关于 µ 和 σ 2 来最⼤ 化函数（1.54）， 但是在⾼斯分布的情况下， µ 的解和 σ 2 ⽆关， 因此我们可以⾸先估计公式 （1.55）然后使⽤这个结果来估计公式（1.56）。
...
我们现在⽤训练数据 { x, t } ，通过最⼤似然⽅法，来决定未知参数 w 和 β 的值。如果数据假定 从分布（1.60）中抽取，那么似然函数为
...
对于确定 w 的问题来说，最⼤化似然函数等价于最⼩化由公式（1.2）$E(w) = \frac{1}{2} \sum^{N}_{n=1}\{y(x_n,w)- tn\}^2$ 定义的平⽅和误差函数。因此，**在⾼斯噪声的假设下，平⽅和误差函数是最⼤化似然函数的⼀个⾃然结果**。
我们也可以使⽤最⼤似然⽅法来确定⾼斯条件分布的精度参数 β 。 关于 β 来最⼤化函数 （1.62）
...
其中 α 是分布的精度，M + 1是对于M 阶多项式的向量 w 的元素的总数。像 α 这样控制模型参数分布的参数，被称为**超参数（hyperparameters）**。
...
$$
因此我们看到**最⼤化后验概率等价于最⼩化正则化的平⽅和误差函数**（之前在公式（1.4）中提到），正则化参数为 λ =α / β 。
...
这⾥，p(t | x, w) 由公式（1.60）给出， 并且我们省略了对于 α 和 β 的依赖， 简化记号。 这⾥，p(w | x, t)是参数的后验分布，可以通过对公式（1.66）归⼀化得到。我们在3.3节将看到， 对于曲线拟合这样的问题，后验分布是⼀个⾼斯分布，可以解析地求出。类似地，公式（1.68）中的积分也可以解析地求解。因此，预测分布由⾼斯的形式给出
$$
...
其中，I 是单位矩阵，向量 ϕ (x)被定义为 $\phi_i(x) = x^i (i = 0, \dots , M)$。

https://www.cnblogs.com/wacc/p/5495448.html


```python
# 图 1.17: ⽤贝叶斯⽅法处理多项式曲线拟合问题得到的预测分布的结果。使⽤的多项式为M = 9，超参数 
# 被固定为 α = 5 × 10 −3 和 β = 11.1（对应于已知的噪声⽅差）。 其中，红⾊曲线表⽰预测概率分
...
这里的$ p(D|w_{ML}) $是最合适的对数似然函数，$ M $是模型中的可调节参数。  
...
$$ p(C_k|x) = \displaystyle\frac{p(x|C_k)p(C_k)}{p(x)} \tag{1.77} $$
...
$$ \begin{split} p(mistake) =& p(x \in R_1, C_2) + p(x \in R_2, C_1) \\ =& \int_{R_1} p(x, C_2)dx + \int_{R_2} p(x, C_1)dx \tag{1.78} \end{split}  $$
...
使⽤乘积规则$p(x, C_k) = p(C_k| x)p(x)$ 来消除共同因⼦$p(x)$。决策规则: 对于每个新的$x$ ，把它分到第$j$类，能使下式取得最⼩值：
...
* 生成模型（generative model）:通过对每个类别$C_k $，独立的确定类别的条件密度$ p(x | C_k) $来解决推断问题，还分别推断出类别的先验概率$　p(C_k) $，然后使用贝叶斯定理：

$$ p(C_k|x) =  \displaystyle\frac{p(x|C_k)p(C_k)}{p(x)} \tag{1.82} $$

来计算类别的后验概率$ p(C_k|x) $。

* 判别模型（discriminative models），解决确定类别的后验密度$p(C_k|x) $的推断问题，然后，使用决策论来对新的输入$ x $进行分类。
...
$$
p(g\mid o) = \frac{p(o\mid g)p(g)}{p(o)}=\frac{p(o\mid g)p(g)}{p(o\mid r)p(r)+ p(o\mid b)p(b)+ p(o\mid g)p(g)}\\
=\frac{0.6\times0.3}{0.6\times0.3+0.2\times0.4+0.2\times0.5}=0.5
$$

1.4 解：

为了说明最大化$p_x(x)$对应的$\hat{x}$ 与最大化$p_y(y)$对应的$\hat{y}$ 并不一定按照简单的函数关系$\hat{x}=g(\hat{y})$相互对应，设$|g'(y)|=sg'(y),s\in[-1,1]$ ，然后对式（1.27）$p_y(y)=p_x(g(y))|g'(y)|$进行微分
$$
...
$$
\mathrm{var}(f(x)) = \mathbb{E}[(f(x)-\mathbb{E}[f(x)])^2]\\=\mathbb{E}[f(x)^2-2f(x)\mathbb{E}[f(x)]+\mathbb{E}^2[f(x)]]\\
...
=\sum_{x}p(x)x\sum_{y}p(y)y=\mathbb{E}[x]\mathbb{E}[y]
$$
对于连续情况也有
$$
\mathbb{E}_{x,y}[xy]=\int_{x}\int_{y}p(x,y)xy\mathrm{d}x\mathrm{d}y\\
=\int_{x}p(x)x\mathrm{d}x\int_{y}p(y)y\mathrm{d}y=\mathbb{E}[x]\mathbb{E}[y]
$$
按照协方差的定义有
$$
...
=\mathbb{E}_{xy}[xy]-\mathbb{E}[x]\mathbb{E}[y]-\mathbb{E}[x]\mathbb{E}[y]+\mathbb{E}[x]\mathbb{E}[y]\\
=\mathbb{E}_{xy}[xy]-\mathbb{E}[x]\mathbb{E}[y]=0
$$
...
=\left[\mu^2+\sigma^2-\frac{2}{N}\left(\mu^2+\sigma^2+(N-1)\mu^2\right)+\frac{1}{N^2}\left(N(\mu^2+\sigma^2)+N(N-1)\mu^2\right)\right]
...
定义
$$
w_{ij}^S=\frac{1}{2}(w_{ij}+w_{ji})\\ w_{ij}^A=\frac{1}{2}(w_{ij}-w_{ji})
$$
