...
期望：$$\mathbb{E}[\mu]=\displaystyle\frac{a}{a+b}\tag{2.3}$$

方差：$$\mathbb{E}[\mu^2] = \displaystyle\frac{ab}{(a+b+1)(a+b)^2}\tag{2.4}$$  
...
\\
(\text{习题1.17})=\frac{\Gamma(a+b)}{\Gamma(a)\Gamma(b)}\cdot \frac{a\Gamma(a)\Gamma(b)}{(a+b)\Gamma(a+b)}
=\frac{a}{a+b}
$$
...
\\
\mathrm{var}[\mu]=\mathbb{E}[\mu^2]-\mathbb{E}[\mu]^2 = \frac{ab}{(a+b+1)(a+b)^2}
$$
...
**多元高斯分布**
$$
\mathcal{N}(\boldsymbol{x} \mid \boldsymbol{\mu}, \boldsymbol{\Sigma})=\frac{1}{(2 \pi)^{\frac{D}{2}}} \frac{1}{|\boldsymbol{\Sigma}|^{\frac{1}{2}}} \exp \left\{-\frac{1}{2}(\boldsymbol{x}-\boldsymbol{\mu})^{T} \boldsymbol{\Sigma}^{-1}(\boldsymbol{x}-\boldsymbol{\mu})\right\} \tag{2.43}
$$
...
根据分块矩阵的逆矩阵的恒等式，换用分块协⽅差矩阵来表示，条件概率$ p(x_a|x_b) $的均值和方差
...
$$
\begin{split} \ln p(z) &= \ln p(x) + \ln p(y|x) \\ &= -\frac{1}{2}(x - \mu)^T\Lambda(x-\mu) \\ &-\frac{1}{2}(y-Ax-b)^TL(y-Ax-b) + const \tag{2.102} \end{split} 
$$
